...
class BST {
    ...
    printValues(node = this.root, separator = ', ', first = true): any {
        if (!node) {
            return '';
        }
        const nodeString = first ? node.val : separator + node.val;

        return `${node.val} ${this.printValues(node.left, separator, false)} ${this.printValues(
            node.right,
            separator,
            false
        )}`;
    ...
    minRecursive(current = this.root): any {
        if (current === null) {
            return null;
        }
    ...
    containsRecursive(searchVal: number, current = this.root): any {
        if (current === null) {
            return false;
        }
