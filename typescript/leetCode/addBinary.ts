...
const addBinary: (s1: string, s2: string) => string = (s1: string, s2: string) =>
    (parseInt(s1, 2) + parseInt(s2, 2)).toString(2); // doesn't actually work

const addBinaryManually: (s1: string, s2: string) => string = (bs1: string, bs2: string) => {
    let sum = '',
        carry = 0,
        i = bs1.length - 1,
        j = bs2.length - 1;
    ...
    while (i >= 0 || j >= 0) {
        const a: string = bs1[i] ? bs1[i] : '0';
        const b: string = bs2[j] ? bs2[j] : '0';
