...
const twoSum1 = (nums: any[], target: number) => {
    const seen: any = {};
    for (let i = 0; i < nums.length; i++) {
...
const twoSum2 = (nums: any[], target: number) => {
    const seen: { [key: string]: any } = {};
    for (let i = 0; i < nums.length; i++) {
        const complement = target - nums[i];
        if (seen.hasOwnProperty(complement)) return [i, seen[complement]];
        if (!seen.hasOwnProperty(nums[i])) {
            seen[nums[i]] = i;
        }
    }
